...
TestClass::TestClass(std::string name, bool use_file) :
	m_name(name),
	m_file(use_file ? name : ""),
	m_use_file(use_file)
{ }
...
const TSPFile& TestClass::RunTests(size_t num_of_ittr, std::chrono::seconds timeout) {
...
void TestClass::_test(size_t num_of_ittr) {
	size_t i = 0;
...
void TestClass::writeToFile(double time, size_t i)
{
	if (m_use_file) {
		m_file.writeFile();
